...
portfolio_weights = pd.DataFrame(data= opt_result.x *100 ,columns=["Portfolio weight"]).set_index(data.columns)

fig1, ax1 = plt.subplots()
ax1.pie(portfolio_weights["Portfolio weight"], labels=portfolio_weights.index, autopct='%1.1f%%', startangle=90) 
# Equal aspect ratio ensures that pie is drawn as a circle.
st.pyplot(fig1)

return_vol_sharpe = pd.DataFrame(get_ret_vol_sr(opt_result.x), index=["Return","Volatility","Sharp-ratio"],columns=["Portfolio KPIs:"])
st.write(return_vol_sharpe)
